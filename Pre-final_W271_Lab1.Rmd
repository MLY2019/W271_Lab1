...
fail.plot <- ggplot(data=results_df, aes(Temp, expected_failures)) +
  geom_point() +
  geom_line(color='blue') +
  ggtitle("Expected number of failures \nas a function of Temperature") +
  theme(plot.title = element_text(lineheight=1, face='bold'))
...
The probability of an O-ring failure at 31? was estimated using the following equation:
$$
...
pi.hat
```
Based on our model, the estimated probability of an O-ring failure at 31? is 0.8178. Next, we looked at both the Wald and LRT confidence intervals for this temperature.
...
Using the parametric bootstrap, at temperatures of 31?, we obtained the 90% confidence interval of (0.1182, 0.9928) and at temperatures of 72?, we obtained the 90% confidence interval of (0.0105, 0.0685). It worths of notice that despite the lower conficence level, the bootstrap intervals were wider than either the Wald or LRT interval.
...
ggplot(mod4, aes(.fitted, .resid)) + geom_point() +
  geom_smooth(method="loess") + geom_abline(intercept = 0, linetype="dashed", slope=0, color='red') +
  xlab("fitted values") + ylab("Residuals") +
  ggtitle("Residuals vs. Fitted") +
  theme(plot.title = element_text(lineheight=1, face='bold'))

ggplot(mod4, aes(.fitted, sqrt(abs(.stdresid)))) + geom_point(na.rm=TRUE) +
