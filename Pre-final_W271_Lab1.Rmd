...
ggplot(results_df, aes(Temp, pi.hat.linear)) +  geom_line(color='red') +
  ...
  ggtitle("Predicted probability of O-ring failure at a given temperature.\n(Linear Model)") +
  theme(plot.title = element_text(lineheight=1, face='bold'))
```
...
As an added point, which may be worth considering in the context of this data at the time of the challenger accident, it does seem as though discounting the influence of pressure and interactions it may have with temperature is not entirely settled. While it is not obviously significant given the current data, it does seem concerning and subsequent error modeling would benefit from collection of addtional test data related to pressure/temperature interactions.
